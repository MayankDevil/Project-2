...
- File: /src/js/lipi.js
- Description: this is class to practice GUPT lipi
*/

export default class GUPT {
    ...
    /*
        -----------------------------------------------------------------
        | Binary : convert text data to binary form (8-bit UTF-16 unit) |
        -----------------------------------------------------------------
    ...
    */ 
    ...
    binary (data) {
        ...
        return data.charCodeAt(0).toString(2).padStart(8, "0")
    }

    /* 
        --------------------------------------
        | Text : convert binary data to text |
        --------------------------------------
    */
    ...
    /* 
        ----------------------------------
        | num : convert binary to number |
        ----------------------------------
    */

    num (data) {

         // remove spaces if string like "0100 0001"
    ...
    */ 
    ...
    bitEnCrypt (data) {
        ...
        data.split('').forEach((value, index) => {
            ...
            let binary = this.binary(value)

            // output.push(binary)
            
            output.push((index % 2 === 0)? binary.slice(1) + binary[0] : binary[binary.length - 1] + binary.slice(0, binary.length - 1))
        })
    ...
    */
    ...
    bitDeCrypt (data) {
        ...
        data.split(' ').forEach((value, index) => {
            ...
            let text = (index % 2 === 0)? value[value.length - 1] + value.slice(0, value.length - 1) : value.slice(1) + value[0] 

            // output.push(this.text(value))
            
            output.push(this.text(text))
        })
    ...
    /*
        -----------------------------------------------------------
        | shiftEnCode : shift ASCII codes for text-based encoding |
        -----------------------------------------------------------
    */
    shiftEnCode(data) {
        return data.split('').map((char, i) => {
            let code = char.charCodeAt(0)
            return String.fromCharCode(code + ((i % 5) + 1)) // shift by 1–5
        }).join('')
    }

    /*
        ---------------------------------------------------------
        | shiftDeCode : restore ASCII codes to original text    |
        ---------------------------------------------------------
    */
    shiftDeCode(data) {
        return data.split('').map((char, i) => {
